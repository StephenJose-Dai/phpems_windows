...
set "COMPOSE_URL=https://github.com/baicu"
...
:ONLINE_PULL
echo.
echo [WARN]  【注意】在线拉取镜像需要配置 VPN，否则可能出现拉取失败的情况。
echo [WARN]  VPN 配置文档请参考：https://mp.weixin.qq.com/s/LXal8PCCYHRPtFDxVmkOYw
echo.
...
if /i "%CONFIRM%"=="y" (
    ...
    if %errorlevel% neq 0 (
        echo [ERROR] 镜像拉取失败，请检查网络或 VPN 配置后重试。
        pause
        exit /b 1
    )
    echo [OK]    镜像拉取完成
